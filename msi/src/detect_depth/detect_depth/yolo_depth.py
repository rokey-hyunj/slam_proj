...
class YoloDepth(Node):
    def __init__(self):
        ...
        # 3. 구독자(Subscriber) 설정: ApproximateTimeSynchronizer 사용
        # message_filters.Subscriber 대신 import한 Subscriber 사용
        self.sub_rgb = Subscriber(self, Image, RGB_TOPIC) 
        self.sub_depth = Subscriber(self, Image, DEPTH_TOPIC)

        # ApproximateTimeSynchronizer 사용: slop=0.1 (0.1초의 시간차 허용)
        self.ats = ApproximateTimeSynchronizer([self.sub_rgb, self.sub_depth], 
                                               queue_size=10, 
                                               slop=0.5) 
        self.ats.registerCallback(self.image_depth_callback)

        # 4. 발행자(Publisher) 설정
        # ReliabilityPolicy.BEST_EFFORT 또는 ReliabilityPolicy.RELIABLE 로 설정
        image_qos = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT, # 또는 RELIABLE
            history=HistoryPolicy.KEEP_LAST,
            depth=1
        )
        self.image_publisher = self.create_publisher(Image, 
                                                    '/oakd/annotated_image_with_depth', 
                                                    qos_profile=image_qos)


    def image_depth_callback(self, rgb_msg, depth_msg):
        self.get_logger().info('=== Callback 시작 ===')
        self.get_logger().info(f'RGB 이미지 크기: {rgb_msg.width}x{rgb_msg.height}')
        self.get_logger().info(f'Depth 이미지 크기: {depth_msg.width}x{depth_msg.height}')
        self.get_logger().info(f'RGB encoding: {rgb_msg.encoding}')
        self.get_logger().info(f'Depth encoding: {depth_msg.encoding}')
        
        try:
            cv_image = self.bridge.imgmsg_to_cv2(rgb_msg, "bgr8")
            self.get_logger().info(f'RGB 변환 성공: {cv_image.shape}')
            
            depth_mm = self.bridge.imgmsg_to_cv2(depth_msg, desired_encoding='passthrough')
            self.get_logger().info(f'Depth 변환 성공: {depth_mm.shape}, dtype: {depth_mm.dtype}')
